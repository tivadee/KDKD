...
class ModelReportCustomer extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT customer_id,CONCAT(firstname, ' ', lastname) AS customer, email, COUNT(order_id) AS orders, SUM(total) AS `total` FROM `oc_order` group by CONCAT(firstname, ' ', lastname) ORDER BY total DESC limit 20";
	...
	public function getOrders2($data = array()) {
		$sql = "SELECT customer_id,CONCAT(firstname, ' ', lastname) AS customer, email, COUNT(order_id) AS orders, SUM(total) AS `total` FROM `oc_order` group by CONCAT(firstname, ' ', lastname) ORDER BY total DESC limit 10";
...
////////////////////////////////////////////////////////////////////////////////
	...
	public function getCustomersOnline2($data = array()) {
		$sql = "SELECT a.zone_id,IFNULL(b.count,0) as total  FROM
oc_zone a  left JOIN (SELECT payment_zone_id,count(payment_zone) as count FROM oc_order group by payment_zone) b
on b.payment_zone_id = a.zone_id";
