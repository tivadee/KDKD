...
class ControllerReportProductViewed extends Controller {
	public function grp() {
			//$this->load->language('report/product_viewed2');

		$this->document->setTitle($this->language->get('กราฟแสดงผู้เข้าชมสินค้า'));
		...
		$url = '';
	$data['grpp'] = $this->url->link('report/product_viewed/grp', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('report/product_viewed2.tpl', $data));
	}
